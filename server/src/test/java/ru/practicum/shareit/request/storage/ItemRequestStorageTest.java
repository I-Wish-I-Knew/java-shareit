...
class ItemRequestStorageTest {
    ...
    void findAllByAuthorId() {
        ...
        int size = 2;
        int page = from / size;
        List<ItemRequest> itemRequests = storage.findAllByAuthorId(user.getId(),
                PageRequest.of(page, size, ItemRequestServiceImpl.SORT));
        Assertions.assertThat(itemRequests).hasSize(1)
                .contains(itemRequest);

        from = 0;
        size = 3;
        page = from / size;
        List<ItemRequest> itemRequests2 = storage.findAllByAuthorId(user2.getId(),
                PageRequest.of(page, size, ItemRequestServiceImpl.SORT));
        Assertions.assertThat(itemRequests2).hasSize(3)
                .contains(itemRequest2)
                .contains(itemRequest4)
    ...
    void findAllByAuthorIdNot() {
        ...
        List<ItemRequest> itemRequests2 = storage.findAllByAuthorIdNot(user2.getId(),
                PageRequest.of(page, size, ItemRequestServiceImpl.SORT));
        Assertions.assertThat(itemRequests2).hasSize(2)
                .contains(itemRequest5)
                .contains(itemRequest3);
    }
}